locals {
  ...
  # Primary parameters
  project_id = local.config.project_id_prd
  location   = local.config.location

  # Foundation IAM
  admins_roles      = local.config.foundation_iam.prd.data_admins
  developers_roles  = local.config.foundation_iam.prd.data_developers
  maintainers_roles = local.config.foundation_iam.prd.data_maintainers
  ...
  cloud_scheduler_job               = local.config.sftp_connector.cloud_scheduler_jobs_prd
  cloud_scheduler_jobs_ony_dataform = local.config.sftp_connector.cloud_scheduler_jobs_only_dataform_prd
...
terraform {
  ...
  backend "gcs" {
    bucket = "bli-bi-commerciale-prod-001-terraform-state-bucket" # This cannot be dynamically computed
