...
resource "google_cloud_scheduler_job" "daily_trigger_job" {
  for_each = { for job in local.config.sftp_connector.cloud_scheduler_jobs_dev : job.table => job }
...
resource "google_cloud_scheduler_job" "daily_trigger_job_only_dataform" {
  for_each = { for job in local.config.sftp_connector.cloud_scheduler_jobs_only_dataform_dev : job.table => job }
